        ...
        private static void TextBox_PreviewTextInput(object sender, TextCompositionEventArgs e)
        {
            ...
            if (textBox.SelectionLength == 0)
            {
                if(textBox.Text.Length < textBox.SelectionStart)
                    newText = textBox.Text.Insert(textBox.Text.Length, $"{Thread.CurrentThread.CurrentCulture.NumberFormat.NumberDecimalSeparator}{e.Text}");

                else newText = textBox.Text.Insert(textBox.SelectionStart, e.Text);
            }
            else
            {
